...
class WPCD_DNS extends WPCD_Base {
	...
	public function set_dns_for_domain( $domain, $ipv4, $ipv6 = '' ) {
		...
		if ( wpcd_get_option( 'wordpress_app_dns_cf_enable' ) ) {
			...
			if ( strpos( $domain, $root_domain ) ) {
				...
				$ip      = $ipv4;
				return $this->cloudflare_add_subdomain( $domain, $zone_id, $token, $ip, $proxied );
			}
		}
	 ...
	 * @return boolean
	 */
	public function cloudflare_add_subdomain( $domain, $zone_id, $token, $ip, $proxied = true ) {
			...
			if ( 200 !== $response_code ) {
				...
				$message = sprintf( __( 'The automatic DNS setup for %s in Cloudflare returned an error. You will need to add this site manually.', 'wpcd' ), $domain );
	 ...
	 * @return boolean
	 */
	public function cloudflare_get_subdomain_details( $domain, $zone_id, $token ) {
	 ...
	 * @return boolean
	 */
	public function cloudflare_get_subdomain_id( $domain, $zone_id, $token ) {
		$details = $this->cloudflare_get_subdomain_details( $domain, $zone_id, $token );
	...
	public function cloudflare_delete_subdomain( $domain, $zone_id, $token ) {
			...
			// Get the domain id for the passed domain.
			$cf_domain_id = $this->cloudflare_get_subdomain_id( $domain, $zone_id, $token );
		...
		if ( ! $cf_domain_id ) {
			...
			if ( 200 !== $response_code ) {
				...
				/* Translators: 1. %s is the subdomain whose details we are attempting to get from cloudflare. 2. %s is the response code we got from cloudflare */
				$message = sprintf( __( 'We were unable to delete domain %1$s in Cloudflare - an eror was returned: %2$s. You will need to add this site manually.', 'wpcd' ), $domain, $response_code );
				do_action( 'wpcd_log_error', $message, 'error', __FILE__, __LINE__, array(), false );
