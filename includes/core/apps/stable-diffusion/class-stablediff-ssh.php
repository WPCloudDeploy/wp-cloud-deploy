...
class STABLEDIFF_SSH extends WPCD_SSH {
	...
	public function construct_error( $message ) {
		$new_message     = '<div class="wpcd-stablediff-error-message-wrap">';
			$new_message = $new_message . '<div class="wpcd-stablediff-error-message-title">' . __( 'Oops - An Error Has Occurred', 'wpcd' ) . '</div>';
			$new_message = $new_message . '<div class="wpcd-stablediff-error-message">' . $message;
				// Add text to tell user how to dismiss the message..
				$new_message = $new_message . '<div class="wpcd-stablediff-error-dismiss-message">' . __( 'To dismiss this message click on the X in the upper right or anywhere on the page outside of this box.', 'wpcd' ) . '</div>';
			$new_message    .= '</div>';

			// add in a link to the help screen if a link was set up in settings...
			$help_url = wpcd_get_option( 'stablediff_general_help_url' );
		if ( ! empty( $help_url ) ) {
			$new_message         .= '<hr />';
			$new_message         .= '<div class="wpcd-stablediff-error-message-help-footer">';
				$new_message     .= '<div class="wpcd-stablediff-error-message-help-title">' . 'Help Resources' . '</div>';
				$new_message     .= '<div class="wpcd-stablediff-error-message-help-text">';
