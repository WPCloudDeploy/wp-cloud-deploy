...
class WPCD_WORDPRESS_TABS_SERVER_USERS extends WPCD_WORDPRESS_TABS {
	...
	public function get_tab( $tabs, $id ) {
		if ( true === $this->wpcd_wpapp_server_user_can( 'view_wpapp_server_users_tab', $id ) && true === $this->wpcd_can_author_view_server_tab( $id, $this->get_tab_slug() ) ) {
	...
	public function tab_action( $result, $action, $id ) {
		...
		if ( in_array( $action, $valid_actions ) ) {
			if ( false === $this->wpcd_wpapp_server_user_can( 'view_wpapp_server_users_tab', $id ) ) {
				return new \WP_Error( sprintf( __( 'You are not allowed to perform this action - permissions check has failed for action %1$s in file %2$s for post %3$s by user %4$s', 'wpcd' ), $action, basename( __FILE__ ), $id, get_current_user_id() ) );
			}
		}
